...
public class KivyLauncher {
	...
	private func pythonHome() {
		...
		let stdlib = resourcePath.appendingPathComponent("python-stdlib")
		let site_packages = resourcePath.appendingPathComponent("site-packages")
		//let python_path = "PYTHONPATH=\(stdlib.path):\(stdlib.path):\(std_dynload.path):\(resourcePath)/site-packages\(extra):."
		let python_path = "PYTHONPATH=\(resourcePath):\(stdlib):\(site_packages)\(extra):."
		putenv(python_path)
	}
